...
class profil extends Controller
{
    public function index()
    {
        if (isset($_SESSION['id'])) {
            ...
            $data['kenyangin'] = $this->model('Barang_model')->getBarangUser($data['user']['id']);
    ...
    public function user($id)
    {
        ...
        $data['kenyangin'] = $this->model('Barang_model')->getBarangUser($id);

        if (empty($data['penjual']) && empty($data['kenyangin'])) {
            header('Location: ' . BASEURL);
            exit;
        } else {
            if (isset($_SESSION['id'])) {
